...
#     backup_dir = check_backup_dir(subfolder)
...
def gen_commit_id(name_len):
    """A function that takes an integer indicating desired length
    and returns a string of random hexadecimal chars at that length."""
    HEX_CHARS = list('1234567890abcdef')
    commit_id = ''
    for _ in range(name_len):
        commit_id += random.choice(HEX_CHARS)
    return commit_id
...
def update_master(main_backup_dir, commit_id):
...
def commit(message):
    ...
    if not (main_backup_dir / 'references.txt').exists():
        with open(main_backup_dir / 'references.txt', 'w') as references:
            references.write(f'HEAD={commit_id}\n'
                             f'master commit={commit_id}')
    update_master(main_backup_dir, commit_id)  #ask about this!
