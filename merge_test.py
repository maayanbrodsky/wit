...
def check_backup_dir(subdir=None):
    ...
    is passed the function returns the '.wit' directory. If the '.wit' directory does
...
def branch_or_commit(user_input, main_backup_dir):
    """Determines if the user passed a branch name or a commit id
...
def find_parent_in_metadata(commit_id):
    """A function that returns the parent in the commit id's metadata file."""
    images = check_backup_dir('images')
    print(len(commit_id))
    filename = f'{commit_id}.txt'
    if filename == 'None.txt':
        return 'None'
    with open(images / filename) as metadata_file:
...
def find_lineage(commit_id):
    """A function that takes a commit id and returns a list of the parent commit id's
    all the way to 'None'."""
    parent_list = [commit_id]
    parent = commit_id
    while parent != 'None':
        parent = find_parent_in_metadata(parent)
        parent_list.append(parent)
    return parent_list


def find_common_id(head_lineage, branch_lineage):
    """A function that loops through the head lineage and the branch lineage and
    returns the id of their common base."""
    for head_id in head_lineage:
        for branch_id in branch_lineage:
            if head_id == branch_id:
                return head_id


def compare_branch_and_common_source(branch_id, common_source):
    """Compares between the most recent commit and the branch and returns
    a list of new files in the branch."""
    images = check_backup_dir('images')
    branch_dir = images / branch_id
    common_source_dir = images / common_source
    head_vs_branch = filecmp.dircmp(common_source_dir, branch_dir)
    return head_vs_branch.right_only


def add_to_staging_area(staging_area, branch_path, diff_files):
    """A function that adds files from the merged branch to the staging area."""
    for item in diff_files:
        item = branch_path / item
        copy_file_or_dir(item, staging_area)


def find_branch_commit_id(main_backup_dir, branch_name):
    """A function that takes a branch name and returns its commit id."""
    commit_dict = create_commit_dict(main_backup_dir)
    if branch_name in commit_dict.keys():
        return commit_dict[branch_name]


def update_merge_metadata(main_backup_dir, new_commit_id):
    activated_branch, _activated_commit_id = active_branch_commit_id(main_backup_dir)
    with open(main_backup_dir / 'references.txt', 'r') as references:
        references_lines = references.readlines()
    references_lines[0] = f'HEAD={new_commit_id}\n'
    new_lines = []
    for line in references_lines:
        separator = line.find('=')
        title = line[:separator]
        new_line = line
        if title == activated_branch:
            new_line = f'{title}={new_commit_id}\n'
        new_lines.append(new_line)
    with open(main_backup_dir / 'references.txt', 'w') as references:
        references.writelines(new_lines)
    with open(main_backup_dir / 'images' / f'{new_commit_id}.txt', 'r') as metadata:
        metadata_lines = metadata.readlines()
    metadata_lines[0] = f'{metadata_lines[0][:-1]} ,{new_commit_id}\n'
    with open(main_backup_dir / 'images' / f'{new_commit_id}.txt', 'w') as metadata:
        metadata.writelines(metadata_lines)
...
def init():
    ...
    new_dir = pathlib.Path() / parent_dir / main_backup_dir / 'images'
...
def add(src):
    ...
    sub_folder = 'staging_area'
    src = pathlib.Path(src)
    src = src.absolute().resolve()
    dst = check_backup_dir(sub_folder)
    copy_file_or_dir(src, dst)


def commit(message):
    """A function that commits the content of the staging area to an image folder
    and generates meta-data files. Returns the commit id for use in merge operations."""
    images = check_backup_dir('images')
    main_backup_dir = check_backup_dir()
...
def checkout(user_input):
    ...
    update_head(main_backup_dir, commit_id)
...
def merge(branch_name):
    """Merges the branch passed to it with the common source of the head
    and updates the metadata"""
    main_backup_dir = check_backup_dir()
    branch_id = find_branch_commit_id(main_backup_dir, branch_name)
    with open(main_backup_dir / 'references.txt', 'r') as references:
        lines = references.readlines()
    head_id = lines[0][5:45]
    branch_lineage = find_lineage(branch_id)
    head_lineage = find_lineage(head_id)
    common_source = find_common_id(head_lineage, branch_lineage)
    diff_files = compare_branch_and_common_source(branch_id, common_source)
    staging_area = check_backup_dir('staging_area')
    branch_path = main_backup_dir / 'images' / branch_id
    add_to_staging_area(staging_area, branch_path, diff_files)
    new_commit_id = commit(f'Commit for merge with {branch_name}')
    update_merge_metadata(main_backup_dir, new_commit_id)
